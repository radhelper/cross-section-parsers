...
def getFluenceFluxDiamond(startDT, endDT):
    # inFile = open(inFileName, 'r')
    # endDT = startDT + timedelta(minutes=60)
    ...
    for l in fileLines:
        ...
        dayTime = lv[1]
        secFrac = lv[2]
        counter_40 = float(lv[3])
        counter_20 = float(lv[4])
        counter_30 = float(lv[5])
        fission_counter = lv[6]
        curIntegral = lv[7]
        current = lv[8]
        ...
        if startDT <= curDt and first_counter_40 is None:
            first_counter_40 = counter_40
            last_counter_20 = counter_20
            last_counter_30 = counter_30
            last_counter_40 = counter_40
            lastDT = curDt
        ...
        if first_counter_40 is not None:
            if counter_30 == last_counter_30:
                shutter = "Closed"
                beamOffTime += (curDt - lastDT).total_seconds()
            else:
        ...
        if curDt > endDT:
            print "curDt " + str(curDt) + "endDt" + str(endDT)
            print "counter_40 " + str(counter_40) + " first " + str(first_counter_40) + " res " + str(
                counter_40 - first_counter_40)
            flux = ((counter_40 - first_counter_40) / (endDT - startDT).total_seconds()) * diamondFactor
            return [flux, beamOffTime]
...
#########################################################
if len(sys.argv) < 5:
    print "Usage: %s <neutron counts input file> <csv file> <factor> <diamondFactor>" % (sys.argv[0])
    sys.exit(1)

inFileName = sys.argv[1]
csvFileName = sys.argv[2]
factor = float(sys.argv[3])
diamondFactor = float(sys.argv[4])
...
while i < len(lines):
    ...
    fluxDiamond, timeBeamOff = getFluenceFluxDiamond(startDT, (startDT + timedelta(minutes=60)))
    fluxAccTime, timeBeamOffAccTime = getFluenceFlux(startDT, (startDT + timedelta(seconds=accTimeS)))
    fluxAccTimeDiamond, timeBeamOffAccTime = getFluenceFluxDiamond(startDT, (startDT + timedelta(seconds=accTimeS)))
    fluence = fluxDiamond * accTimeS
    fluenceAccTime = fluxAccTimeDiamond * accTimeS
    if fluence > 0:
    ...
    headerC.append("Flux 1h Diamond (factor " + str(factor) + ")")
    headerC.append("Flux AccTime Diamond (factor " + str(factor) + ")")
    ...
    row = [startDT.ctime(), endDT.ctime(), (i - j + 1), sdcS, accTimeS, abortZeroS, flux, fluxAccTime, fluxDiamond,
           fluxAccTimeDiamond, fluence, fluenceAccTime, crossSection, crossSectionCrash, timeBeamOff,
           crossSectionAccTime, crossSectionCrashAccTime, timeBeamOffAccTime]
    writer.writerow(row)
